/*
 * SparkSql grammar file based on
 * https://raw.githubusercontent.com/apache/spark/master/sql/catalyst/src/main/antlr4/org/apache/spark/sql/catalyst/parser/SqlBase.g4
 *
 * Modifications:
 * 1. to avoid conflict, rename
 *   complex -> r_complex
 *   from -> r_from
 *   str -> r_str
 *   input -> r_input
 *   len -> r_len
 * 2. remove @parser:: members , modify rules as they should be
 *   public boolean legacy_setops_precedence_enbled = false;
 *   public boolean legacy_exponent_literal_as_decimal_enabled = false;
 *   public boolean SQL_standard_keyword_behavior = false;
 * 3. remove @lexer::members, ignore isValidDecimal() and isHint()
 *
 * 4. modify errorCapturingIdentifier to NOT capture wrong identifiers
 *
 * 5. add alternatives for predicate at line 710
 */
...
statement
    ...
    | ALTER TABLE table=multipartIdentifier
        RENAME COLUMN
        r_from=multipartIdentifier TO to=errorCapturingIdentifier        #renameTableColumn
    | ALTER TABLE multipartIdentifier
        DROP (COLUMN | COLUMNS)
        '(' columns=multipartIdentifierList ')'                        #dropTableColumns
    | ALTER TABLE multipartIdentifier
        DROP (COLUMN | COLUMNS) columns=multipartIdentifierList        #dropTableColumns
    | ALTER (TABLE | VIEW) r_from=multipartIdentifier
    ...
    | ALTER TABLE multipartIdentifier
        r_from=partitionSpec RENAME TO to=partitionSpec                  #renameTablePartition
    | ALTER (TABLE | VIEW) multipartIdentifier
...
queryTerm
    ...
    | left=queryTerm operator=(UNION | EXCEPT | SETMINUS) setQuantifier? right=queryTerm              #setOperation
    ;
...
predicate
    : NOT? kind=BETWEEN lower=valueExpression AND upper=valueExpression                             #bewteen
    | NOT? kind=IN '(' expression (',' expression)* ')' #inList
    | NOT? kind=IN '(' query ')' #inSubquery
    | NOT? kind=RLIKE pattern=valueExpression #rlike
    | NOT? kind=LIKE quantifier=(ANY | SOME | ALL) ('('')' | '(' expression (',' expression)* ')') #likeList
    | NOT? kind=LIKE pattern=valueExpression (ESCAPE escapeChar=STRING)? #likeValue
    | IS NOT? kind=NULL #nullPredicate
    | IS NOT? kind=(TRUE | FALSE | UNKNOWN) #booleanPredicate
...
primaryExpression
    ...
    | POSITION '(' substr=valueExpression IN r_str=valueExpression ')'                           #position
    ...
    | (SUBSTR | SUBSTRING) '(' r_str=valueExpression (FROM | ',') pos=valueExpression
      ((FOR | ',') r_len=valueExpression)? ')'                                                   #substring
    | TRIM '(' trimOption=(BOTH | LEADING | TRAILING)? (trimStr=valueExpression)?
       FROM srcStr=valueExpression ')'                                                         #trim
    | OVERLAY '(' r_input=valueExpression PLACING replace=valueExpression
...
unitToUnitInterval
    : value=intervalValue r_from=identifier TO to=identifier
    ;
...
dataType
    : r_complex=ARRAY '<' dataType '>'                            #complexDataType
    | r_complex=MAP '<' dataType ',' dataType '>'                 #complexDataType
    | r_complex=STRUCT ('<' complexColTypeList? '>' | NEQ)        #complexDataType
